...
export class ParagonService {
  ...
  fetchParagonHistory(): Observable<IParagon[]> {
    return this.http.get<IParagon[]>(API_URL + 'Paragon/GetByUsername?' + 'username=' + this._appUserService.currentUser.Username);
  }
  ...
  SaveNewParagon(paragon: IParagon) {
    ...
    this.http.post<IParagon>(API_URL + 'Paragon/AddNew', body, requestOptions).subscribe(data => {
